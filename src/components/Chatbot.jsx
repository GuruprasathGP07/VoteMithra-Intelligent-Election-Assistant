...
import { isFirebaseConfigured, db, ref, push } from '../services/firebaseService';
...
const Chatbot = () => {
  ...
  CRITICAL: The user interface is currently set to ${getLanguageName(i18n.language)}. 
  YOU MUST RESPOND ENTIRELY IN ${getLanguageName(i18n.language)}. 
  IGNORE the language of any previous messages in this history if they are different. 
  If the user asks a question in a different language, answer it IN ${getLanguageName(i18n.language)} anyway. 
  Do not explain why you are switching, just switch immediately to ${getLanguageName(i18n.language)}.`;
  ...
  const handleSend = async (text = null) => {
    ...
    const newMessages = [...messages, { role: "user", parts: [{ text: msgText }] }];
    ...
    try {
      ...
      const responseText = await sendMessage(msgText, i18n.language, messages, systemPrompt);
      const updatedMessages = [...newMessages, { role: "model", parts: [{ text: responseText }] }];
      ...
      setMessages([...newMessages, { role: "model", parts: [{ text: t('chatbot.error_message') || "I am currently overloaded. Please try again." }] }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <>
      ...
      <button 
        ...
        aria-label={isOpen ? "Close Chatbot" : "Open AI Voter Coach"}
      >
      ...
      >
        ...
        <div className="bg-saffron p-4 flex items-center gap-3 text-blue-main border-bottom-2 border-white/20">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-bold font-dm-sans leading-none">{t('chatbot.title')}</h4>
            <p className="text-[10px] opacity-80 mt-1">{t('chatbot.subtitle')}</p>
          </div>
        ...
        >
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] p-3 text-[13px] rounded-2xl shadow-sm ${
                m.role === 'user' 
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="bg-blue-pale p-3 rounded-2xl rounded-bl-none flex gap-1" data-testid="loading-spinner">
