...
const Guidance = () => {
  ...
  const docCategories = [
    { title: t('guide.docs_identity'), options: t('guide.docs_options_id'), icon: 'fingerprint' },
    { title: t('guide.docs_address'), options: t('guide.docs_options_address'), icon: 'map' },
    { title: t('guide.docs_age'), options: t('guide.docs_options_age'), icon: 'calendar_today' },
    { title: t('guide.docs_photo'), options: t('guide.docs_photo'), icon: 'photo_camera' }
  ];
    ...
    <div className="bg-bg-main min-h-screen pb-20">
      ...
      <div className="bg-blue-main text-white py-16 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto space-y-4 relative z-10">
          <Link to="/" className="inline-flex items-center gap-2 text-blue-pale font-bold hover:text-white transition-colors">
            <span className="material-icons text-sm">arrow_back</span> {t('nav.home')}
          </Link>
          <h1 className="text-5xl font-extrabold font-playfair">{t('guide.title')}</h1>
          <p className="text-xl text-blue-pale max-w-2xl">{t('guide.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-10 space-y-12">
        ...
        <div className="space-y-6">
          <h2 className="text-3xl font-bold text-blue-main font-playfair text-center">{t('guide.eligibility_title')}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {eligibilityItems.map((item, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-blue-pale/20 text-center hover:bg-blue-pale/10 transition-colors">
                <span className="material-icons text-4xl text-saffron mb-3">{item.icon}</span>
        ...
        <div className="bg-blue-pale/20 rounded-[40px] p-10 space-y-8">
          <h2 className="text-3xl font-bold text-blue-main font-playfair text-center">{t('guide.docs_title')}</h2>
          <div className="grid md:grid-cols-2 gap-6">
            {docCategories.map((doc, idx) => (
              <div key={idx} className="flex gap-4 p-6 bg-white rounded-3xl shadow-sm border border-white hover:border-blue-main transition-all">
                ...
                <div>
                  <h4 className="font-bold text-blue-main mb-1 text-lg">{doc.title}</h4>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-6">
            <div className="flex items-center gap-4 bg-saffron/10 p-4 rounded-2xl">
                <span className="material-icons text-saffron text-3xl">language</span>
                <h3 className="text-2xl font-bold text-blue-main">{t('guide.apply_online')}</h3>
            </div>
            ...
            <div className="relative pl-8 space-y-8 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-blue-pale">
              {onlineSteps.map((step, idx) => (
                <div key={idx} className="relative flex flex-col gap-1 group">
                  ...
                  {idx === 0 && (
                      <a href="https://voters.eci.gov.in/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1 text-sm">
                        voters.eci.gov.in <span className="material-icons text-xs">open_in_new</span>
                      </a>
                  )}
                </div>
              ))}
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-8 rounded-[30px] shadow-lg border border-blue-pale/30 space-y-6">
              <h3 className="text-2xl font-bold text-blue-main flex items-center gap-3">
                <span className="material-icons">store_mall_directory</span> {t('guide.apply_offline')}
              </h3>
              <ul className="space-y-4">
                <li className="flex gap-3">
                  <span className="material-icons text-blue-main">check_circle</span>
                  <span className="font-medium text-ink">{t('guide.offline_visit')}</span>
                </li>
                <li className="flex gap-3">
                  <span className="material-icons text-blue-main">check_circle</span>
                  <span className="font-medium text-ink">{t('guide.offline_form')}</span>
                </li>
              </ul>
              
              <div className="pt-6 border-t border-blue-pale/30 text-center">
                <h4 className="font-bold text-blue-main mb-4">{t('guide.processing_title')}</h4>
                <div className="flex justify-around items-center">
                  <div className="text-center">
                    <span className="text-3xl font-black text-saffron block">2-4</span>
                    <span className="text-xs uppercase font-bold tracking-widest opacity-60">{t('guide.time_title')}</span>
                  </div>
            ...
            <div className="bg-gradient-to-br from-blue-main to-indigo-900 p-8 rounded-[30px] text-white space-y-4 shadow-xl">
                <h3 className="text-xl font-bold font-playfair flex items-center gap-3">
                    <span className="material-icons text-saffron">stars</span> {t('guide.approval_title')}
                </h3>
        ...
        <div className="bg-red-50 border-2 border-red-100 rounded-3xl p-10 space-y-8 relative overflow-hidden">
            <div className="absolute -right-10 -bottom-10 opacity-5 pointer-events-none">
                <span className="material-icons text-[200px] text-red-main">warning</span>
            </div>
          <h2 className="text-3xl font-bold text-red-main font-playfair">{t('guide.mistakes_title')}</h2>
          <div className="grid sm:grid-cols-2 gap-6 relative z-10">
            {mistakes.map((mistake, idx) => (
              <div key={idx} className="flex items-center gap-4 bg-white/60 p-4 rounded-xl border border-red-50 group hover:bg-white transition-all">
                <span className="material-icons text-red-main group-hover:scale-110 transition-transform">cancel</span>
