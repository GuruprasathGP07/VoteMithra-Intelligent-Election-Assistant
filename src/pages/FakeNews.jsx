...
const FakeNews = () => {
  ...
  const swipes = [
    {
      ...
      type: "FAKE",
      explanation: "ECI never changes voting dates via WhatsApp. Verify at eci.gov.in.",
    },
    {
      id: 2,
      text: "⚠️ Breaking: EVMs in 5 states found pre-loaded with votes. Supreme Court ordered re-election. Forward urgently.",
      type: "FAKE",
      explanation: "No such Supreme Court order exists. Common misinformation pattern.",
    },
    {
      id: 3,
      text: "Your EPIC number is required to vote. Without it you CANNOT enter the booth.",
      type: "FAKE",
      explanation: "Partially false. 12 alternative documents are accepted by ECI. Voter ID is not the only option.",
    },
  ];
  ...
  const getRecommendedAction = (riskLevel) => {
    if (riskLevel === "SAFE") return "This content appears credible. Always cross-check with official ECI sources.";
    if (riskLevel === "FAKE") return "Do NOT share this message. Report it via cVIGIL app or call 1950.";
  ...
  const analyzeMessage = async () => {
    ...
    try {
      ...
      const trustScore = typeof result.score === "number"
        ? result.score
      ...
      setAnalysisResult({
        trustScore,
        riskLevel,
        explanation: result.reasoning || result.explanation || "Analysis complete.",
        flaggedKeywords: result.flaggedKeywords || [],
        recommendedAction: result.recommendedAction || getRecommendedAction(riskLevel),
      });
    } catch (error) {
      console.error("Analysis error:", error);
      setAnalysisError("Analysis failed. Please check your internet connection and try again.");
    } finally {
  ...
  return (
    <main data-testid="fake-news-page" id="main-content" className="max-w-4xl mx-auto py-10 px-6 space-y-10">
      ...
      {phase === 1 && (
        <div className="card max-w-lg mx-auto animate-fadeIn" role="region" aria-label="Swipe Challenge">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold">Swipe Challenge</h3>
            <span className="text-sm font-bold text-saffron" aria-label={`Question ${currentSwipe + 1} of ${swipes.length}`}>
          ...
          {!swipeFeedback ? (
            <div className="space-y-6">
              <div className="bg-[#E1FCEF] p-6 rounded-radius border border-[#A1E8C6] drop-shadow-sm" role="article">
                <div className="flex items-center gap-2 text-[10px] text-[#064E3B] font-bold uppercase mb-2">
                  <span className="material-icons text-xs" aria-hidden="true">forward</span>
                  Forwarded many times
                </div>
                <p className="text-[#064E3B] font-medium leading-relaxed">{swipes[currentSwipe].text}</p>
              </div>
            ...
            <div className="space-y-6" aria-live="polite">
              <div className={`p-4 rounded-radius text-center font-bold text-lg ${swipeFeedback.isCorrect ? "text-green-600" : "text-red-600"}`}>
                {swipeFeedback.isCorrect ? "✓ Correct!" : "✗ Incorrect"}
              </div>
              <p className="text-sm text-center italic">{swipeFeedback.explanation}</p>
              <button
                type="button"
                className="btn-primary w-full focus:ring-4"
                onClick={nextSwipe}
              >
                {currentSwipe === swipes.length - 1 ? "Go to Custom AI Scanner" : "Next Message →"}
              </button>
            </div>
          )}
        </div>
      ...
      {phase === 2 && (
        <div className="card max-w-2xl mx-auto animate-fadeIn" role="region" aria-label="Custom AI Scanner">
          <h3 className="text-xl font-bold mb-2">Custom AI Message Scanner</h3>
          <p className="text-sm text-muted mb-6">
            Paste any election-related message to analyze its credibility using Gemini AI.
          </p>

          <div className="space-y-4">
            <label htmlFor="news-input" className="sr-only">Paste news message here</label>
            <textarea
              id="news-input"
            ...
            <button
              ...
              aria-label={isAnalyzing ? "Analyzing news content with AI..." : "Analyze news content with Gemini AI"}
            >
          ...
          {analysisError && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-radius text-red-700 text-sm flex items-center gap-2" role="alert">
              <span className="material-icons text-base" aria-hidden="true">error_outline</span>
              {analysisError}
            </div>
          )}
              ...
              return (
                <div className="mt-10 pt-10 border-t border-border-gray animate-fadeIn space-y-6">
                  ...
                  <div className="flex items-center gap-6">
                    <div className={`w-24 h-24 rounded-full border-8 flex items-center justify-center text-2xl font-black ${colors.border} ${colors.text}`}
                      aria-label={`Trust score: ${analysisResult.trustScore} out of 100`}>
                      {analysisResult.trustScore}
                    </div>
                    <div>
                      <div className={`inline-block px-3 py-1 rounded text-xs font-bold mb-2 uppercase tracking-widest ${colors.badge}`}>
                  ...
                  <div className="bg-bg-main p-4 rounded-radius space-y-2">
                    <p className="text-xs font-bold text-muted uppercase">Analyzed Message</p>
                    <p className="text-sm italic text-muted leading-relaxed">
                      {customInput.split(" ").map((word, i) => {
                        const isFlagged = analysisResult.flaggedKeywords?.some(
                          (kw) => word.toLowerCase().includes(kw.toLowerCase())
                        );
                        return (
                          <span key={i} className={isFlagged ? "bg-red-100 text-red-700 px-0.5 rounded" : ""}>
                            {word}{" "}
                          </span>
                        );
                      })}
                  ...
                  <div className="bg-blue-pale p-4 rounded-radius flex items-center gap-3 text-blue-main font-bold">
                    ...
                    <span className="text-sm">{analysisResult.recommendedAction}</span>
                  </div>
                  ...
                  <button
                    ...
                    onClick={() => { setAnalysisResult(null); setCustomInput(""); setAnalysisError(null); }}
                  >
