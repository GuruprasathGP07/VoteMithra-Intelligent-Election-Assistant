...
const Home = () => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative min-h-[calc(100vh-64px)] bg-blue-main flex flex-col items-center justify-center overflow-hidden px-8 py-12 text-center text-white">
        <div className="mt-8 mb-6 relative w-[300px] h-[380px] md:w-[340px] md:h-[460px] rounded-[100px] overflow-hidden border-4 border-white/20 shadow-[-10px_10px_40px_rgba(0,0,0,0.3)] animate-fadeIn shrink-0">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-blue-main/80 backdrop-blur pb-6 pt-12 mt-auto text-center [mask-image:linear-gradient(to_bottom,transparent,black_40%)]">
            <p className="text-saffron font-bold text-[15px] tracking-widest uppercase px-2 leading-tight">{t('hero.title')}</p>
          </div>
        </div>
      ...
      {/* Journey Map Section */}
      <section id="journey-section" className="py-20 px-8 max-w-6xl mx-auto w-full">
        <div className="text-center mb-16 space-y-2">
          <h2 className="text-3xl md:text-4xl text-blue-main font-bold font-playfair">{t('journey.title')}</h2>
          <p className="text-muted">{t('journey.subtitle')}</p>
        </div>

        <div className="relative min-h-[500px]">
          {/* Desktop S-Curve SVG */}
          <svg className="hidden md:block absolute inset-0 w-full h-full pointer-events-none z-0" viewBox="0 0 900 400" preserveAspectRatio="none">
            <path
            ...
            {journeyStops.map((stop) => (
              ...
              >
                <div className="relative w-12 h-12 bg-white border-2 border-border-gray rounded-full flex items-center justify-center text-blue-main shadow-main shrink-0">
                  <span className="material-icons text-2xl leading-none">{stop.icon}</span>
      ...
      <section className="py-20 px-8 max-w-6xl mx-auto w-full">
        <h2 className="text-3xl font-bold text-blue-main mb-10 font-playfair">{t('awareness.title')}</h2>
      ...
      <section className="py-20 px-8 max-w-4xl mx-auto w-full">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-blue-main font-playfair mb-4">{t('faq.section_title')}</h2>
          <p className="text-muted">{t('faq.section_subtitle')}</p>
        </div>

        <div className="space-y-4">
          {homeFaqs.map((faq, index) => (
            <div key={index} className="card p-0 overflow-hidden border border-border-gray">
              <button
                className="w-full text-left p-6 flex justify-between items-center bg-white hover:bg-bg-main transition-colors"
                onClick={() => setOpenFaq(openFaq === index ? null : index)}
              >
                <span className="font-bold text-blue-main">{t(`faq_items.${faq.k}.q`)}</span>
                <span className={`material-icons transition-transform ${openFaq === index ? 'rotate-180' : ''}`}>
                  expand_more
                </span>
              </button>
        ...
        <div className="mt-12 text-center">
          ...
          >
            {t('faq.see_all')} <span className="material-icons">arrow_forward</span>
          </button>
        </div>
      </section>
